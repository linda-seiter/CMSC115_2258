...
public class InventoryAnalyzerSolution {
    ...
    public static String getItemReorderSummary(String inventoryData, String itemName) {
        ...
        String reorderStatus = quantityOnHand <= reorderThreshold ? "needs to be reordered"
                : "does not need to be reordered";

        // Return a formatted item summary, including the reorder status
        return String.format("%s - Quantity On Hand: %d, Reorder Threshold: %d - %s", itemName, quantityOnHand,
                reorderThreshold, reorderStatus);
    }
